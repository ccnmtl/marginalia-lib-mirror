...
function getBlockParent( node, root )
{
	...
	{
		if ( ELEMENT_NODE == node.nodeType && 'block' == htmlDisplayModel( node.tagName ) )
			return node;
		node = node.parentNode;
	}
	return root;
...
function stripMarkup( node, tagName, className, doNormalize, callback )
{
	...
	while ( null != child )
	{
		...
		{
			stripMarkup( child, tagName, className, false, callback );
			if ( ( null == tagName || child.tagName.toUpperCase( ) == tagName.toUpperCase( ) )
				&& ( null == className || hasClass( child, className ) ) )
			{
				if ( null != callback )
					callback( child );
